...
def get_editorial(problem_id: UUID, db: Session = Depends(get_db),
...
def update_editorial(editorial_id: UUID, data: schemas.EditorialUpdate, db: Session = Depends(get_db),
                     current_user: User = Depends(require_role(["admin", "moderator"]))):
...
def delete_editorial(editorial_id: UUID, db: Session = Depends(get_db),
