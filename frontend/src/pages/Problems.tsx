      ...
      {tags && (tags as { id: string; name: string }[]).length > 0 && (
        <div className="flex flex-wrap gap-2 mb-5">
          {(tags as { id: string; name: string }[]).slice(0, 15).map(tag => (
            <button key={tag.id} className="px-3 py-1 text-xs rounded-full border border-[var(--border)] bg-[var(--surface)] text-app-3 hover:border-purple-400 hover:text-purple-600 hover:bg-[var(--accent-light)] transition-all cursor-pointer">
              {tag.name}
            </button>
          ))}
        </div>
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-app overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="border-b border-[var(--border)] bg-[var(--surface-2)]">
              <th className="text-left px-5 py-3 text-xs font-semibold text-app-3 uppercase tracking-wider w-12">#</th>
              <th className="text-left px-5 py-3 text-xs font-semibold text-app-3 uppercase tracking-wider">{t.problems.title}</th>
              <th className="text-left px-3 py-3 text-xs font-semibold text-app-3 uppercase tracking-wider">{t.problems.difficulty}</th>
              <th className="text-right px-5 py-3 text-xs font-semibold text-app-3 uppercase tracking-wider">{t.problems.solved}</th>
            ...
            {isLoading
              ...
              : filtered.map((p: ProblemShort, i: number) => (
                  <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                    ...
                    <td className="px-5 py-4 text-app-3 text-sm font-mono">{page * limit + i + 1}</td>
                    <td className="px-5 py-4">
                      <Link to={`/problems/${p.slug}`} className="text-app-1 hover:text-purple-600 dark:hover:text-purple-400 font-medium text-sm transition-colors flex items-center gap-2">
                        <Code2 size={14} className="text-app-3 shrink-0" />{p.title}
                      </Link>
                    </td>
                    <td className="px-3 py-4"><DifficultyBadge difficulty={p.difficulty} /></td>
                    <td className="px-5 py-4 text-right text-sm text-app-3 font-mono">{p.solve_count.toLocaleString()}</td>
                    <td className="px-3 py-4 text-right">
                      <button onClick={e => toggleBm(e, p.id)} className="text-app-3 hover:text-yellow-500 transition-colors cursor-pointer opacity-0 group-hover:opacity-100">
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="py-16 text-center text-app-3">
            <Code2 size={40} className="mx-auto mb-3 opacity-20" /><p>{t.problems.not_found}</p>
          </div>
        )}
      </div>

      <div className="flex items-center justify-between mt-4">
        <Button variant="outline" size="sm" onClick={() => setPage(Math.max(0, page - 1))} disabled={page === 0}>← {t.common.back}</Button>
        <span className="text-app-3 text-sm">{page + 1}</span>
        <Button variant="outline" size="sm" onClick={() => setPage(page + 1)} disabled={(problems?.length ?? 0) < limit}>{t.common.next} →</Button>
      </div>
    </div>
  );
}
